...
namespace SOFIE{
   ...
   RModel_GNN& RModel_GNN::operator=(RModel_GNN&& other){
    ...
    void RModel_GNN::Generate(){
        ...
        for(int k=0; k<num_edges; ++k){
            fGC+="std::copy(Edge_"+std::to_string(k)+"_Update.begin(),Edge_"+std::to_string(k)+"_Update.end(),input_graph.edge_data.GetData()+"+std::to_string(k*num_edge_features)+");\n";
        }
        fGC+="\n";
#endif
#if 0
        // update edges (generate in a vector)
        fGC += "for (int k = 0; k < " + std::to_string(num_edges) + "; k++) { \n";
        fGC += "   std::vector<float> Edge_Update_k = ";
        fGC += edges_update_block->Generate({"input_graph.edge_data.GetData()+ k * "+std::to_string(num_edge_features_input),
                                            "input_graph.node_data.GetData()+ fReceivers[k] * " + std::to_string(num_node_features_input),
                                            "input_graph.node_data.GetData()+ fSenders[k] * " + std::to_string(num_node_features_input),
                                            "input_graph.global_data.GetData()"});
        fGC += "\n";
        fGC += "   std::copy(Edge_Update_k.begin(),Edge_Update_k.end(),fEdgeUpdates.begin()+ k * " + std::to_string(num_edge_features) + ");\n";
        fGC += "}\n";
        ...
        fGC+="\n";
#endif
        // optimize edges combining in a single call to edge_update

        // for (int k = 0; k < 4348; k++) {
        //     std::copy(input_graph.edge_data.GetData() + k * 100, input_graph.edge_data.GetData() + (k + 1) * 100,
        //               fV1.begin() + k * 100);
        //     std::copy(input_graph.node_data.GetData() + fReceivers[k] * 100,
        //               input_graph.node_data.GetData() + (fReceivers[k] + 1) * 100, fV2.begin() + k * 100);
        //     std::copy(input_graph.node_data.GetData() + fSenders[k] * 100,
        //               input_graph.node_data.GetData() + (fSenders[k] + 1) * 100, fV3.begin() + k * 100);
        //     std::copy(input_graph.global_data.GetData(), input_graph.global_data.GetData() + 100,
        //               fV4.begin() + k * 100);
        //     // std::vector<float> Edge_Update_k = edge_update.infer(input_graph.edge_data.GetData()+ k *
        //     // 100,input_graph.node_data.GetData()+ fReceivers[k] * 100,input_graph.node_data.GetData()+ fSenders[k] *
        //     // 100,input_graph.global_data.GetData());
        //     // std::copy(Edge_Update_k.begin(),Edge_Update_k.end(),fEdgeUpdates.begin()+ k * 50);
        // }
        // // std::vector<float> EdgeUpdates = edge_update.infer(fV1.data(),fV2.data(),fV3.data(),fV4.data());
        // edge_update.infer(fV1.data(), fV2.data(), fV3.data(), fV4.data(), fEdgeUpdates.data());
        // //  resize edge graph data since output feature size is not equal to input size
        // input_graph.edge_data = input_graph.edge_data.Resize({4348, 50});
        // for (int k = 0; k < 4348; k++) {
        //     std::copy(fEdgeUpdates.begin() + k * 50, fEdgeUpdates.begin() + (k + 1) * 50,
        //               input_graph.edge_data.GetData() + k * 50);
        // }
